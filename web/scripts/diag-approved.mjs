...
page.on('request', req => {
  ...
  if (url.includes('/chapters/ch-3/content') && req.method() === 'GET') {
    return respond(200, { data: { chapter_spec_id: 'ch-3', version: 1, content_text: '# test', word_count: 100, min_word_met: true } })
  }
...
const diag = await page.evaluate(() => {
  ...
  return {
    headerApproveAllBtn: allButtons.some(b => b.textContent && b.textContent.includes('全部通过')),
    headerAdvanceBtn: allButtons.some(b => b.textContent && b.textContent.includes('进入审计')),
    headerReviewChip: !!Array.from(document.querySelectorAll('span')).find(s => s.textContent && s.textContent.includes('已审') && s.textContent.includes('2/3')),
    approvedBadgeInTree: Array.from(document.querySelectorAll('span')).filter(s => s.textContent && s.textContent.trim() === '已审').length,
    inspectorApproveBtn: allButtons.some(b => b.textContent && b.textContent.includes('审核通过此章节')),
    inspectorRejectBtn: allButtons.some(b => b.textContent && b.textContent.includes('驳回')),
