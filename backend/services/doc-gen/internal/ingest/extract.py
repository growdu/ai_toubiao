...
def fallback_docx(path):
    """python-docx 缺失时，直接读 word/document.xml 剥标签。"""
    import zipfile
    try:
        ...
        return {"text": "", "tables": [], "warnings": [f"docx 降级失败: {e}"]}
...
def extract_pdf(path):
    try:
        ...
        return {"text": "", "tables": [], "warnings": ["pdf: pdfplumber 缺失"], "needs_ocr": True}
    parts = []
    tables = []
    try:
        ...
        return {"text": "", "tables": [], "warnings": [f"pdf 提取异常: {e}"], "needs_ocr": True}
    text = "\n\n".join(parts)
...
def fallback_xlsx(path):
    ...
    try:
        ...
        return {"text": "", "tables": [], "warnings": [f"xlsx 降级失败: {e}"]}


def extract_xls(path):
    # xlrd 2.x 不再支持 .xls；需 libreoffice 转换，环境缺失则跳过。
...
def main():
    ...
    if fn is None:
        ext = "." + fmt if fmt else ""
        res = extract_text_file(path) if (ext in TEXT_EXTS) else extract_text_file(path)
    else:
        try:
            res = fn(path)
        except Exception as e:
            res = {"text": "", "tables": [], "warnings": [f"{fmt} 提取失败: {e}\n{traceback.format_exc()}"]}
